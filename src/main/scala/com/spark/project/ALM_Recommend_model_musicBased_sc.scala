...
//Считаем значение AUC для каждого пользователя

    // Определяем данные для кросс валидации как положительные
    // Делаем предсказание для всех таких данных
    val positivePredictions = predictFunction(positiveData.select("user", "artist")).
      withColumnRenamed("prediction", "positivePrediction")


    // Создание набора негативных рекомендаций для пользователей, формируется случайно из всех прослушиваний за исключением тех, который попали в положительный список
    val negativeData = positiveData.select("user", "artist").as[(Int,Int)].
      ...
      flatMapGroups { case (userID, userIDAndPosArtistIDs) =>
        ...
        while (i < allArtistIDs.length && negative.size < posItemIDSet.size) {
          ...
          // Добавляем только новые уникальные ID
    ...
    // Делаем предсказание для оставшихся:
    val negativePredictions = predictFunction(negativeData).
      withColumnRenamed("prediction", "negativePrediction")

    // Соединяем негативные и позитивные предсказания по пользователю
    //
    val joinedPredictions = positivePredictions.join(negativePredictions, "user").
      select("user", "positivePrediction", "negativePrediction").cache()

    // Считаем количества пар по пользователям
    val allCounts = joinedPredictions.
      groupBy("user").agg(count(lit("1")).as("total")).
      select("user", "total")
    // Считаем количество правильно выбранных пар по пользователям
    val correctCounts = joinedPredictions.
      filter($"positivePrediction" > $"negativePrediction").
      groupBy("user").agg(count("user").as("correct")).
      select("user", "correct")

    // Комбинируем их, вычисляем среднее значение, отношение
...
// Функция которая предсказывает наиболее популярных
